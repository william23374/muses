...
module.exports = createDomainHandler({
  ...
  members: {
    baseURL: GATEWAY_BASE,
    // The concept version room member pop-up window uses the audience list of Zhongle room; rmservice/members
    // What is returned is the continuous microphone seat, and ordinary listeners who enter the room will not appear in it.
  ...
  recent_playlist: {
    ...
    method: 'POST',
    params: (p) => ({ roomid: p.roomid || p.room_id || '' }),
    // The conceptual version signs this interface and sends an empty request body, not a JSON object.
    data: () => '',
  },
  order_song: {
    baseURL: GATEWAY_BASE,
