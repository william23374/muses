...
class CustomTrayMenuService {
    ...
    hide() {
        ...
        this.finishRenderWait();
        // Destroy window after 5 minutes idle to free memory
        clearTimeout(this.idleDestroyTimer);
    ...
    normalizeSong(currentSongRaw, playerProgressRaw) {
        ...
        try {
            ...
            return {
                title: currentSong?.displayName || currentSong?.name || 'Muses',
                artist: currentSong?.author || 'Muses',
    ...
    async handleAction(action) {
        ...
        if (keepOpenActions.has(action)) {
            // Fallback refresh; normally driven by current-song-updated events
            clearTimeout(this.refreshTimer);
