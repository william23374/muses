...
// Expose extension management API
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    systemVolumeSupported: () => ipcRenderer.invoke('system-volume-supported'),
    getSystemVolume: () => ipcRenderer.invoke('get-system-volume'),
    setSystemVolume: (volume) => ipcRenderer.invoke('set-system-volume', volume),
    setSystemMuted: (muted) => ipcRenderer.invoke('set-system-muted', muted),
    // Music API bridge: the renderer routes requests here (instead of the local
    // HTTP server) when running inside Electron with the default/local API URL.
    musicApi: (payload) => ipcRenderer.invoke('music-api', payload),
});
