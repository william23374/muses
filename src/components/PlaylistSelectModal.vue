...
const validateUserAndSong = () => {
    ...
    if (Array.isArray(props.currentSong)) {
        if (!props.currentSong[0].hash) {
            window.$modal.alert(t('mei-you-xuan-ze-zheng-que-de-ge-qu'));
            return false;
        }
    ...
    if (props.currentSong.isCloud) {
        window.$modal.alert(t('yun-pan-yin-le-bu-zhi-chi-tian-jia-dao-ge-dan'));
        return false;
    }
    if (props.currentSong.isLocal) {
        window.$modal.alert(t('ben-di-yin-le-bu-zhi-chi-tian-jia-dao-ge-dan'));
        return false;
    }
    return true;
};
...
const fetchPlaylists = async () => {
    try {
        ...
        playlists.value = playlistResponse.data.info.filter(
            playlist => playlist.list_create_userid === MusesAuth.UserInfo.userid
        );
        isOpen.value = true;
    } catch (error) {
        $message.error(t('huo-qu-ge-dan-shi-bai'));
...
const toLike = () => {
    ...
    if (!like_id) { window.$modal.alert(t('xian-qu-kan-kan-ni-de-shou-cang-jia-ba')); return; }
