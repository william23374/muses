    ...
    <teleport to="body">
        <div v-if="show" class="birthday-fullscreen" aria-hidden="true">
            ...
            <div class="ribbon-wrapper">
                ...
                <div class="ribbon">
                    <h1><span v-if="nickname">{{ nickname }},</span>{{ $t('sheng-ri-kuai-le') }}</h1>
                    <p>{{ $t('yuan-ni-yong-you-mei-hao-yi-tian') }}</p>
                </div>
            </div>
        </div>
    </teleport>
...
const tryAutoPlay = async () => {
    ...
    try {
        await add(props.songHash, t('zhu-sheng-ri-kuai-le', { name: props.nickname || t('ni') }), './assets/images/ico.png');
    } catch {
    }
};
