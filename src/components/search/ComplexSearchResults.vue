<template>
    <div class="complex-search-results">
        <div v-if="visibleSectionCount === 0" class="complex-empty">
            {{ $t('zan-wu-zong-he-sou-suo-jie-guo') }}
        </div>
        ...
        <template v-else>
            ...
            <section v-if="songSection" class="complex-section">
                <div class="section-header">
                    <h3>{{ $t('sou-suo-dan-qu') }}</h3>
                    <button class="section-link" @click="goToSearch('song')">
                        {{ $t('cha-kan-geng-duo-shu-liang', { n: songSection.total || songSection.lists.length }) }}
                    </button>
                </div>
            ...
            <section v-if="albumSection" class="complex-section">
                <div class="section-header">
                    <h3>{{ $t('sou-suo-zhuan-ji') }}</h3>
                    <button class="section-link" @click="goToSearch('album')">
                        {{ $t('cha-kan-geng-duo-shu-liang', { n: albumSection.total || albumSection.lists.length }) }}
                    </button>
                </div>
            ...
            <section v-if="playlistSection" class="complex-section">
                <div class="section-header">
                    <h3>{{ $t('sou-suo-ge-dan') }}</h3>
                    <button class="section-link" @click="goToSearch('collect')">
                        {{ $t('cha-kan-geng-duo-shu-liang', { n: playlistSection.total || playlistSection.lists.length }) }}
                    </button>
                </div>
            ...
            <section v-if="mvSection" class="complex-section">
                <div class="section-header">
                    ...
                    <button class="section-link" @click="goToSearch('mv')">
                        {{ $t('cha-kan-geng-duo-shu-liang', { n: mvSection.total || mvSection.lists.length }) }}
                    </button>
                </div>
                    ...
                    <div v-for="(program, index) in programSection.lists"
                        ...
                        <div class="program-info">
                            ...
                            <div class="program-meta">
                                <span>{{ $t('shou-ge-qu', { n: program.songcount }) }}</span>
                    ...
                    <article v-for="(item, index) in musicPhotoSection.lists"
                        ...
                        <div class="article-info">
                            ...
                            <div class="article-meta">
                                <span>{{ $t('count-yue-du', { n: formatCount(item.scan_num) }) }}</span>
                                <span>{{ $t('count-dian-zan', { n: formatCount(item.like_num) }) }}</span>
                            </div>
                        </div>
                    </article>
                </div>
...
const formatCount = (value) => {
    ...
    if (count < 100000000) return t('count-wan', { n: (count / 10000).toFixed(1) });
    return t('count-yi', { n: (count / 100000000).toFixed(1) });
};
