...
export default function useLocalMusicQueue(t, musicQueueStore, currentSong, timeoutId) {
    ...
    // Add local music to queue and play
    const addLocalMusicToQueue = async (localItem, isReset = true) => {
        ...
        if (typeof window !== 'undefined' && typeof window.electron !== 'undefined') {
            window.electron.ipcRenderer.send('set-tray-title', (localItem.displayName || localItem.name) + ' - ' + (localItem.author || t('wei-zhi-ge-shou')));
        }
        ...
        try {
            ...
            currentSong.value.author = localItem.author || t('wei-zhi-ge-shou');
    ...
    // Batch add local music to playlist
        ...
        try {
            ...
            for (const item of localSongs) {
                ...
                const localSong = {
                    ...
                    author: item.author || t('wei-zhi-ge-shou'),
