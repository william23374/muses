    ...
    <teleport to="body">
        <div v-if="showOnboardingGuide && currentSteps.length" class="onboarding-overlay">
            ...
            <img class="onboarding-mascot" :class="{ intro: isIntroStep }" :style="mascotStyle"
                src="/assets/images/mascotGirl.png" :alt="$t('muses-kan-ban-niang')">
            <section class="onboarding-card" :class="{ centered: !hasTarget, intro: isIntroStep }" :style="cardStyle">
                ...
                <template v-if="isIntroStep">
                    <h3>{{ $t('xin-shou-yin-dao-greeting') }}</h3>
                    <p>{{ $t('xin-shou-yin-dao-intro') }}</p>
                </template>
                <template v-else>
                ...
                <div class="onboarding-actions">
                    <button class="text-button" type="button" @click="skip">{{ $t('tiao-guo') }}</button>
                    <div class="step-actions">
                        <button v-if="!isIntroStep" type="button" :disabled="currentIndex === 0" @click="previousStep">{{ $t('shang-yi-bu') }}</button>
...
const progressText = computed(() => isIntroStep.value ? t('xin-shou-yin-dao') : `${currentIndex.value + 1} / ${currentSteps.value.length}`);
const primaryButtonText = computed(() => {
    if (isIntroStep.value) return t('kai-shi-yin-dao');
    return isLastStep.value ? t('wan-cheng') : t('xia-yi-bu');
});
