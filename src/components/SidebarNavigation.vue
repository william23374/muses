<template>
    <div class="side-top-actions" :class="{ collapsed: isCollapsed }">
        <div class="side-action-buttons">
            <button class="side-action-button" :title="$t('zhe-die-zhan-kai')" @click="toggleCollapse">
                <i :class="isCollapsed ? 'fas fa-indent' : 'fas fa-outdent'"></i>
            </button>
            <button class="side-action-button" :title="$t('hou-tui')" @click="goBack" :disabled="!canGoBack">
                <i class="fas fa-chevron-left"></i>
            </button>
            <button class="side-action-button" :title="$t('qian-jin')" @click="goForward" :disabled="!canGoForward">
                <i class="fas fa-chevron-right"></i>
            </button>
            <button class="side-action-button" :title="$t('shua-xin-ye-mian')" @click="refreshPage">
                <i class="fas fa-redo"></i>
            </button>
            <button class="side-action-button side-recognize-button" :title="$t('ting-ge-shi-qu')" @click="router.push('/recognize')">
    ...
    <aside class="side-navigation" :class="{ collapsed: isCollapsed }">
        <div class="side-profile">
            <router-link :to="!isCollapsed ? '/library' : '/settings'" class="side-profile-link">
                ...
                <div class="side-profile-info">
                    <span>{{ MusesAuth.UserInfo?.nickname || $t('deng-lu') }}</span>
                    <small>{{ MusesAuth.isAuthenticated ? $t('huan-ying-hui-lai') : 'Muses' }}</small>
                </div>
            </router-link>
        ...
        <transition name="side-profile-menu">
            <div v-if="showProfile" class="side-profile-menu" @click.stop="toggleProfile">
                <ul>
                    ...
                    <li>
                        <a v-if="MusesAuth.isAuthenticated" @click="logout">
        ...
        <div class="side-navigation-main">
            ...
            <div class="side-section">
                ...
                <router-link to="/CloudDrive" class="side-link" :title="$t('wo-de-yun-pan')">
                    <i class="fas fa-cloud"></i>
                    <span>{{ $t('wo-de-yun-pan') }}</span>
                </router-link>
                <router-link to="/LocalMusic" class="side-link" :title="$t('ben-di-yin-le')">
                    <i class="fas fa-compact-disc"></i>
                    <span>{{ $t('ben-di-yin-le') }}</span>
                </router-link>
                <router-link to="/recognize" class="side-link" :title="$t('ting-ge-shi-qu')">
                    <i class="fas fa-microphone"></i>
                    <span>{{ $t('ting-ge-shi-qu') }}</span>
                </router-link>
            </div>

            <div v-if="MusesAuth.isAuthenticated" class="side-playlist-area">
                <div class="side-playlist-tabs">
                    ...
                    <button class="side-playlist-toggle-button" :title="activePlaylistTab === 'own' ? $t('shou-cang-ge-dan') : $t('wo-de-ge-dan')"
                ...
                <div class="side-playlist-list">
                    <router-link
                        ...
                        :title="displayPlaylistName(playlist.name)">
                        <div
                            v-if="playlistCoverStyle(playlist)"
                            class="side-playlist-cover system-cover"
                            :class="playlistCoverStyle(playlist)"
                        >
                            <i :class="playlistCoverIcon(playlist)"></i>
                        </div>
                        <img
                            v-else
                            :src="$getCover(playlist.pic, 32)"
                            alt=""
                        >
...
const getplaylist = async () => {
    ...
    try {
        ...
        userPlaylists.value = sortedInfo.filter(playlist => {
            ...
            return playlist.list_create_userid === MusesAuth.UserInfo.userid || isLikedPlaylistName(playlist.name);
        ...
        collectedPlaylists.value = sortedInfo.filter(playlist =>
            playlist.list_create_userid !== MusesAuth.UserInfo.userid && !playlist.authors
        );

        const collectedIds = [];
        sortedInfo.forEach(playlist => {
            if (playlist.list_create_userid !== MusesAuth.UserInfo.userid) {
                collectedIds.push({
...
watch(() => MusesAuth.isAuthenticated, (isAuthenticated) => {
    if (isAuthenticated) {
        void getplaylist();
    } else {
        userPlaylists.value = [];
...
.side-navigation,
.side-top-actions {
    --side-surface: rgba(255, 255, 255, 0.86);
    --side-border: rgba(0, 0, 0, 0.06);
    --side-muted: #86868b;
    --side-text: #1d1d1f;
    --side-action-icon: #6e6e73;
    --side-hover: rgba(0, 0, 0, 0.04);
}

.side-navigation:is(.dark .side-navigation),
.side-top-actions:is(.dark .side-top-actions) {
    --side-surface: rgba(28, 28, 30, 0.86);
    --side-border: rgba(255, 255, 255, 0.08);
    --side-muted: rgba(235, 235, 245, 0.55);
    --side-text: rgba(255, 255, 255, 0.92);
    --side-action-icon: rgba(235, 235, 245, 0.7);
