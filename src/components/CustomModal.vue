...
// Component code from Muses Store (Muses.cn) © 阿珏酱
// window.$modal.alert('This is an Alert'); // Call window.$modal directly
// const result = await window.$modal.confirm('This is a Confirm');
// const result = await window.$modal.prompt('Enter content:', 'default value');
// window.$modal.showLoading(); // open
// window.$modal.hideLoading(); // close
...
const confirmAlert = () => {
    ...
    alertResolve(); // Resolve on OK click to continue subsequent code
};
