...
const newAlbumTypes = computed(() => [
    { label: t('tui-jian'), value: '' },
    { label: t('hua-yu'), value: '1' },
    { label: t('ou-mei'), value: '2' },
    { label: t('ri-ben'), value: '3' },
    { label: t('han-guo'), value: '4' }
]);
...
const cardItems = computed(() => {
    return albumList.value.map(album => ({
        ...
        description: [album.singername, album.publishtime?.split(' ')[0], album.songcount ? t('shou-ge-qu', { n: album.songcount }) : '']
            .filter(Boolean)
            .join(' · ')
    }));
});
...
const normalizeAlbumType = (type) => {
    ...
    return newAlbumTypes.value.some(option => option.value === normalizedType) ? normalizedType : '';
};
