...
export const useStickyDetailHeader = () => {
    ...
    const visualHeaderHeight = computed(() =>
        EXPANDED_HEIGHT - (EXPANDED_HEIGHT - COLLAPSED_HEIGHT) * progress.value
    );
    ...
    const headerStyle = computed(() => ({
        top: 0,
        height: `${visualHeaderHeight.value}px`,
        overflow: 'hidden',
        alignItems: progress.value > 0.55 ? 'center' : 'flex-start'
    }));

    const spacerStyle = computed(() => ({
        height: `${Math.max(0, EXPANDED_HEIGHT - visualHeaderHeight.value)}px`
    }));

    const coverStyle = computed(() => {
        const size = currentCoverSize.value;
        return {
            width: `${size}px`,
            height: `${size}px`,
            '--system-cover-icon-size': `${Math.max(14, size * 0.36)}px`
        };
    });

    const infoStyle = computed(() => ({
        height: `${Math.min(currentCoverSize.value, visualHeaderHeight.value - HEADER_VERTICAL_PADDING / 2)}px`,
        overflow: progress.value > 0 ? 'hidden' : 'visible',
        justifyContent: progress.value > 0.55 ? 'center' : 'flex-start'
    }));

    const titleStyle = computed(() => ({
        fontSize: `${currentTitleSize.value}px`,
        transform: `translateY(${progress.value * 2}px)`
    }));
    ...
    const listHeaderStyle = computed(() => ({
        top: `${visualHeaderHeight.value}px`
    }));

    const trackHeaderStyle = computed(() => ({
        top: `${visualHeaderHeight.value + listHeaderHeight.value - TRACK_HEADER_OVERLAP}px`
    }));
