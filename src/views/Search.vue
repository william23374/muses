...
const searchTabs = computed(() => [
    { type: 'complex', name: t('sou-suo-zong-he') },
    { type: 'song', name: t('sou-suo-dan-qu') },
    { type: 'special', name: t('sou-suo-ge-dan') },
    { type: 'album', name: t('sou-suo-zhuan-ji') },
    { type: 'mv', name: 'MV' },
    { type: 'author', name: t('sou-suo-ge-shou') }
]);

// Switch search type
const changeSearchType = (type) => {
    if (searchType.value === type) return;
    currentPage.value = 1; // Reset page on type switch
...
const displayedPageNumbers = computed(() => {
    const delta = 2; // Page numbers shown before/after current
    let pages = [];
    ...
    if (totalPages.value <= 7) {
        ...
        // Compute middle page number range
...
const handleMvClick = async (mv) => {
    try {
        ...
        const title = mv?.MvName || mv?.FileName || t('shi-pin-bo-fang');
