<template>
    <div class="lyrics-container" :class="{ 'locked': isLocked, 'hovering': isHovering && !isLocked }"
            ...
            <div class="controls-wrapper" :class="{ 'locked-controls': isLocked }">
                <template v-if="!isLocked">
                    <div class="color-controls">
                        <button class="color-button" :title="$t('mo-ren-yan-se')" @click="$refs.defaultColorInput.click()">
                            <div class="color-preview" :style="{ backgroundColor: defaultColor }"></div>
                        </button>
                        <button class="color-button" :title="$t('gao-liang-yan-se')" @click="$refs.highlightColorInput.click()">
                    ...
                    <button @click="changeFontSize(-2)" class="font-control" :title="$t('jian-xiao-zi-ti')">
                        <i class="fas fa-minus"></i>
                        <i class="fas fa-font"></i>
                    </button>
                    <button @click="sendAction('previous-song')" :title="$t('shang-yi-shou')">
                        <i class="fas fa-step-backward"></i>
                    </button>
                    <button @click="togglePlay" :title="isPlaying ? $t('zan-ting') : $t('bo-fang')">
                        <i :class="isPlaying ? 'fas fa-pause' : 'fas fa-play'"></i>
                    </button>
                    <button @click="sendAction('next-song')" :title="$t('xia-yi-shou')">
                        <i class="fas fa-step-forward"></i>
                    </button>
                    <button @click="changeFontSize(2)" class="font-control" :title="$t('zeng-da-zi-ti')">
                        <i class="fas fa-font"></i>
                        <i class="fas fa-plus"></i>
                    </button>
                    <button @click="toggleLock" class="lock-button" :title="isLocked ? $t('jie-suo') : $t('suo-ding')">
                        <i :class="isLocked ? 'fas fa-lock' : 'fas fa-lock-open'"></i>
                    </button>
                    <button @click="sendAction('close-lyrics')" :title="$t('guan-bi-ge-ci')">
        ...
        <div class="lyrics-content-wrapper" ref="lyricsContainerRef" :class="{ 'locked': isLocked }"
            ...
            <div v-else class="lyrics-content hovering nolyrics">{{ $t('ting-ni-xiang-ting') }}</div>
        </div>
        </div>
    </div>
</template>
...
// On Linux, setIgnoreMouseEvents forward unavailable — avoid swallowing clicks
...
const startDrag = (event) => {
    ...
    // Drag only when hovered (must hit lyrics text first)
    if (isHovering.value) {
...
const checkMousePosition = (event) => {
    ...
    if (isLocked.value) {
        // Check if mouse on lyrics text or controls
        ...
        // Show controls when mouse on lyrics or control bar
    ...
    // Use reliable mouse position check
    ...
    // Check mouse on lyrics or control bar
    ...
    // Activate hover when on lyrics or control bar
    if ((isMouseOnLyrics || isMouseInControls) && !isLocked.value) {
        isHovering.value = true
        startHoverPolling()
    }

    // Reset hover only when mouse leaves container
    if (!isMouseInContainer && !isLocked.value) {
        isHovering.value = false
    }

    // Set mouse passthrough; disable in control/hover areas
...
onMounted(() => {
    document.documentElement.classList.add('lyrics-route')
...
onBeforeUnmount(() => {
    document.documentElement.classList.remove('lyrics-route')
    stopHoverPolling()
