        ...
        <section class="recognize-panel">
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <button class="hero-play card-action" type="button" :title="$t('bo-fang')" @click.stop="playSong(item.match.song)">
                            ...
                            <span class="hero-tools">
                                <button class="card-action" type="button" :title="$t('jia-ru-ge-dan')" @click.stop="addSelectedToPlaylist(item.match.song)">
                                    <i class="fas fa-plus"></i>
                                </button>
                                <button class="card-action" type="button" :title="$t('bo-fang')" @click.stop="playSong(item.match.song)">
                                    <i class="fas fa-play"></i>
                                </button>
                                <button class="card-action" type="button" :title="$t('jia-ru-shou-cang')" @click.stop="likeSelectedSong(item.match.song)">
                                    <i class="far fa-heart"></i>
                                </button>
                                <button class="card-action" type="button" :title="$t('sou-suo')" @click.stop="searchSelectedMatch(item.match.song)">
            ...
            <div class="recognize-actions">
                <button v-if="matches.length || status === 'failed'" type="button" @click="reset">
                    <span v-if="matches.length">{{ $t('mei-you-man-yi-de-jie-guo') }}</span>
                    {{ $t('zhong-xin-shi-bie') }}
                </button>
            </div>
        </section>
...
const selectedMicLabel = computed(() => {
    if (!selectedMicId.value) return t('mo-ren-mai-ke-feng');
    const currentDevice = micDevices.value.find(device => device.deviceId === selectedMicId.value);
    return currentDevice ? getMicDeviceLabel(currentDevice) : t('mo-ren-mai-ke-feng');
});

const statusText = computed(() => {
    if (status.value === 'recording') return t('zheng-zai-ling-ting', { s: seconds.value });
    if (status.value === 'recognizing') return t('shi-bie-zhong-qing-shao-hou');
    if (status.value === 'failed') return errorMsg.value || t('wei-shi-bie-dao-ge-qu');
    return source.value === 'mic' ? t('kao-jin-yin-yuan-hou-dian-ji') : t('xuan-ze-gong-xiang-dai-sheng-yin');
});
...
const getMicDeviceLabel = device => device.label || t('mai-ke-feng');
...
const getSystemStream = async () => {
    if (!navigator.mediaDevices?.getDisplayMedia) throw new Error(t('bu-zhi-chi-xi-tong-yin-pin'));
    ...
    if (!stream.getAudioTracks().length) throw new Error(t('wei-huo-qu-xi-tong-yin-pin'));
    return stream;
};
...
const recognizeBlob = async (stream) => {
    if (!chunks.length) {
        stopTracks(stream);
        status.value = 'failed';
        errorMsg.value = t('wei-lu-zhi-dao-yin-pin');
        return;
    }
    ...
    try {
        ...
        errorMsg.value = result.length ? '' : t('wei-shi-bie-dao-qing-zhong-shi');
    } catch (error) {
        console.error('识曲失败:', error);
        status.value = 'failed';
        errorMsg.value = t('shi-bie-guo-cheng-chu-cuo');
    } finally {
        stopTracks(stream);
    }
};
...
const startRecording = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
        status.value = 'failed';
        errorMsg.value = t('bu-zhi-chi-lu-yin');
        return;
    }
    ...
    try {
        ...
        clearTimer();
        status.value = 'failed';
        errorMsg.value = error instanceof Error ? error.message : t('wu-fa-fang-wen-yin-pin-shu-ru');
    }
};
