    ...
    <div class="settings-page">
        ...
        <div v-if="isSelectionOpen" class="modal" @click.self="closeSelection">
            <div class="modal-content">
                ...
                <input v-if="isFontSelection()" class="font-search" :placeholder="$t('sou-suo-zi-ti')"
                    v-model="fontSearch" />
                ...
                <ul v-else-if="selectionType === 'audioOutputDevice'">
                    <li v-if="audioOutputDevicesLoading">{{ $t('zheng-zai-huo-qu-she-bei') }}</li>
                    <li v-else-if="audioOutputDeviceOptions.length === 0">{{ $t('wei-jian-ce-dao-yin-pin-shu-chu-she-bei') }}</li>
                ...
                <div v-if="selectionType === 'apiBaseUrlMode' && selectedSettings.apiBaseUrlMode.value === 'custom'"
                    ...
                    <div class="api-setting-item">
                        <input type="text" v-model="apiBaseUrlForm.url" class="api-input"
                            :placeholder="t('rpc-di-zhi-placeholder', { url: defaultApiBaseUrl })" />
                    </div>
...
const refreshSettingDisplayTexts = () => {
    settingSections.value.forEach((section) => {
        section.items.forEach((item) => {
            const current = selectedSettings.value[item.key];
            if (!current) return;

            if (isFontSelection(item.key)) {
                selectedSettings.value[item.key] = {
                    ...current,
                    displayText: current.value || t('mo-ren-zi-ti')
                };
                return;
            }

            if (item.key === 'apiBaseUrlMode') {
                selectedSettings.value[item.key] = {
                    ...current,
                    displayText: current.value === 'custom' ? t('zi-ding-yi') : t('mo-ren')
                };
                return;
            }

            if (item.key === 'audioOutputDevice' && (!current.value || current.value === 'default')) {
                selectedSettings.value[item.key] = {
                    ...current,
                    displayText: t('mo-ren')
                };
                return;
            }

            if (item.options?.length) {
                const option = item.options.find((opt) => opt.value === current.value);
                if (option) {
                    selectedSettings.value[item.key] = { ...option };
                }
            } else if (item.defaultDisplayText !== undefined && !current.value) {
                selectedSettings.value[item.key] = {
                    ...current,
                    displayText: item.defaultDisplayText
                };
            }
        });
    });
...
const updateAudioOutputDeviceDisplayText = async (deviceId) => {
    if (!deviceId || deviceId === 'default') {
        selectedSettings.value.audioOutputDevice = { displayText: t('mo-ren'), value: 'default' };
        return;
    }

    let displayText = t('yi-xuan-ze-she-bei', { id: deviceId.slice(0, 8) });
    try {
...
const loadAudioOutputDevices = async () => {
    ...
    try {
        ...
        const options = [{ displayText: t('mo-ren'), value: 'default' }];
        let unnamedIndex = 1;

        for (const output of outputs) {
            if (!output.deviceId) continue;
            const displayText = output.label || t('shu-chu-she-bei', { n: unnamedIndex++ });
        ...
        audioOutputDeviceOptions.value = [{ displayText: t('mo-ren'), value: 'default' }];
    } finally {
        ...
        for (const font of fonts) {
            ...
            if (!familyMap.has(family) || name.length < familyMap.get(family).name.length) {
                familyMap.set(family, {
                    // If fullName is too long it is a variant name — use family name instead
...
const selectActions = {
    ...
    updateAudioOutputWatch: async (option) => {
        if (option.value === 'on') {
            ...
            if (!granted) {
                ...
                window.$modal.alert(t('yin-pin-quan-xian-shen-qing-shi-bai'));
                return;
            }
        }
    ...
    handleLogAction: async (option) => {
        ...
        if (result?.error) {
            console.error(`Log action ${option.value} failed:`, result.error);
            window.$modal.alert(t('ri-zhi-cao-zuo-shi-bai'));
        }
        if (option.value === 'export-log' && result?.filePath) {
            await window.$modal.alert(t('ri-zhi-yi-dao-chu', { path: result.filePath }));
        }
    }
};
    ...
    if (savedSettings) {
        ...
        for (const key in savedSettings) {
            ...
            if (key === 'apiBaseUrlMode') {
                ...
                selectedSettings.value[key] = {
                    displayText: value === 'custom' ? t('zi-ding-yi') : t('mo-ren'),
                    value: value
                };
                continue;
            }
...
const testApiBaseUrl = async () => {
    ...
    if (!validation.ok) {
        apiBaseUrlForm.testResult = t(validation.error);
    ...
    if (result.ok) {
        apiBaseUrlForm.testResult = t('lian-jie-cheng-gong');
        apiBaseUrlForm.testStatus = 'success';
    } else if (result.error === 'timeout') {
        apiBaseUrlForm.testResult = t('lian-jie-chao-shi');
        apiBaseUrlForm.testStatus = 'error';
    } else if (result.error === 'no_dfid') {
        apiBaseUrlForm.testResult = t('rpc-duan-dian-xie-yi-bu-fu-he');
        apiBaseUrlForm.testStatus = 'error';
    } else if (typeof result.status === 'number') {
        apiBaseUrlForm.testResult = t('lian-jie-shi-bai', {
            detail: `${result.status} ${result.statusText || ''}`.trim()
        });
        apiBaseUrlForm.testStatus = 'error';
    } else {
        apiBaseUrlForm.testResult = t('lian-jie-cuo-wu', { detail: result.error || '' });
        apiBaseUrlForm.testStatus = 'error';
    }
};
...
const saveApiBaseUrl = () => {
    const validation = validateApiBaseUrl(apiBaseUrlForm.url);
    if (!validation.ok) {
        window.$modal.alert(t(validation.error));
        return;
    }

    const value = validation.value;
    if (!value) {
        selectedSettings.value.apiBaseUrlMode = { displayText: t('mo-ren'), value: 'default' };
        selectedSettings.value.apiBaseUrl = { displayText: '', value: '' };
    } else {
        selectedSettings.value.apiBaseUrlMode = { displayText: t('zi-ding-yi'), value: 'custom' };
...
const testProxyConnection = async () => {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            ...
            proxyForm.testResult = t('lian-jie-cuo-wu', { detail: error.message });
        }
        proxyForm.testStatus = 'error';
    } finally {
...
const recordShortcut = (e) => {
    ...
    // Update hint when modifier pressed
    ...
    // End recording only when key is not modifier alone
...
// Update saveShortcuts with validation
const saveShortcuts = () => {
    if (!isElectron()) {
...
.settings-page {
    ...
    box-shadow: none;
    border-radius: 18px;
    border: 1px solid var(--apple-separator, $border-light);
...
.settings-sidebar {
    ...
    flex: 0 0 220px;
    box-shadow: none;
    border-right: 1px solid var(--apple-separator, $border-light);
    padding: 20px 0;
    overflow-y: auto;
