<template>
    <div class="recommended-search-page">
        ...
        <section class="hero">
            <h1 class="hero-title" v-html="$t('jin-tian-xiang-ting-shen-me-html')"></h1>
            <p class="hero-subtitle">{{ $t('sou-suo-ying-xiong-fu-biao-ti') }}</p>
            ...
            <div ref="searchAreaRef" class="query-area">
                <form class="query-pill" @submit.prevent="submitSearch(searchKeyword)">
                    ...
                    <input v-model.trim="searchKeyword" type="text" :placeholder="$t('sou-suo-placeholder')"
                    ...
                    <button v-if="searchKeyword" type="button" class="query-clear" :title="$t('qing-kong')"
                ...
                <transition name="suggest-pop">
                    <div v-if="showSuggestions" class="suggest-panel">
                        <div v-if="isSuggestLoading" class="suggest-state">
                            ...
                            {{ $t('zheng-zai-huo-qu-sou-suo-jian-yi') }}
                        </div>
                        <template v-else>
        ...
        <section class="trend-section">
            ...
            <div v-else class="trend-empty">
                ...
                <h3>{{ $t('zan-wu-re-sou-shu-ju') }}</h3>
                <p>{{ $t('wang-luo-kai-xiao-cha') }}</p>
...
const normalizeBoards = (list = []) => {
    ...
    return list
        .map((board) => ({
            name: board?.name || t('re-men-bang-dan'),
...
const fetchHotBoards = async () => {
    ...
    try {
        ...
        if (response?.status !== 1) {
            throw new Error(t('re-men-sou-suo-jia-zai-shi-bai'));
        }
