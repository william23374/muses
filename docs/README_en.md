...
<h2 align="center" style="font-weight: 600">Muses</h2>
  <p align="center">
...
* Note: Please open the corresponding port on the server after deployment, or use a reverse proxy for domain access.
  ...
  ```
  docker run -d --name Muses -p 8080:8080 -p 6521:6521 -e PORT=6521 -e platform=lite iajue/muses:latest
  ```
  ...
  * Remote image, version may be behind the official
  ...
  services:
    muses:
    # Image address
    image: registry.cn-wulanchabu.aliyuncs.com/youngxj/muses:latest
    container_name: muses # Container name
  ...
  Copy the content above and paste it into the container orchestration in the Baota panel, name the orchestration as Muses, and click deploy.
